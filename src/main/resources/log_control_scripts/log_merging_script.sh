...
# 2) /log_backups 경로에 있는 파일들은 이제 전부 작업대상임

# 3) /log_backups 경로에 있는 파일들은 하나씩 까면서 내용을 log_aggr 경로에 있는 날짜별 파일로 복사
# #### 날짜 패턴까지만 해서 해당 날짜 파일이 없으면 만들고 시작
# #### 해당 날짜 파일이 있으면 해당 파일에 내용 추가

# 4) 해당 날짜 마지막 파일 병합 완료 시 S3로 보내고 파일 삭제

# 이 작업 주기는 30분 이내로만 수행하면 됨 (logback 이 1분에 하나씩 30개 유지하니까)
